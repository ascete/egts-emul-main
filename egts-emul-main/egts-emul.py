...
class EGTStrack(object):
    ...
    def add_service(self, record_types, *args, **kwargs):
        ...
        if record_types == 1:
            ...
            logging.info(f"llflags {llflags}")
            flags = llflags.to_bytes(1, byteorder='little')
            spd = int(kwargs['speed']).to_bytes(2, byteorder='little')  # speed
            ...
            src = b'\x01'  # 1 byte Источник сообщения "по таймеру при включенном зажигании"
            # Собираем пакет
            ...
            rl = len(_service).to_bytes(2, byteorder='little')  # record length
            headService = rl + rn + rfl + _oid + tm + sst + rst
            logging.info(f"length bytes {len(headService)}")
            logging.info(f"new massage service: {headService}")
    ...
    def new_message(self):
        ...
        logging.info(f"number packet^', {self._pid}")
        if self._service != None:
        ...
        self._service = None
        logging.info(f"new massage {getBytes}")
        logging.info(f"length bytes {len(getBytes)}")
        return getBytes
        pass
    ...
    def bytes(self):
        ...
        getBytes += self._pt
        logging.info(f"length bytes head: {len(getBytes)}")
        return getBytes
        pass

    def __str__(self):
        logging.info(f"massage hex {self.new_message().hex()}")
